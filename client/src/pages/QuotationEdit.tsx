...
export default function QuotationEdit() {
  ...
  const quotationId = parseInt(window.location.pathname.split('/')[2]);
  ...
  // 加载报价单数据（只在quotationId有效时查询）
  const { data: quotationData, isLoading } = trpc.quotations.getById.useQuery(
    { id: quotationId },
    { enabled: !isNaN(quotationId) && quotationId > 0 }
  );
  ...
  const updateQuotationMutation = trpc.quotations.update.useMutation({
    onSuccess: () => {
      toast.success(`报价单更新成功，报价单号：${quotationNumber}`, {
        duration: 5000, // 显示5秒
      });
      // 留在当前编辑页面，不跳转
    },
    onError: (error) => {
      ...
      if (error.message.includes('customer')) {
        ...
        errorMessage = '报价单不存在或已被删除';
      } else {
        errorMessage = `更新报价单失败: ${error.message}`;
      }
  ...
  // 报价单不需要推荐价格功能，已注释
  // const { data: recommendedPriceData, refetch: refetchRecommendedPrice } = trpc.quotations.getRecommendedPrice.useQuery(
  //   {
  ...
  useEffect(() => {
    if (quotationData) {
      const order = quotationData as any; // Type assertion to access all order fields
      setQuotationNumber(order.quotationNumber);
      setCustomerId(order.customerId);
      setCurrency(order.currency);
      // 注意：不再从 quotationData 加载联系人信息，而是等待 letterhead 自动填充
      // setContactPerson(quotationData.contactPerson || "");
      // setContactPhone(quotationData.contactPhone || "");
      // setContactEmail(quotationData.contactEmail || "");
      // setShippingAddress(order.shippingAddress || "");
      setNotes(order.notes || "");
      
      // 加载订单项数据
      if (quotationData.items) {
        console.log('[QuotationEdit] Order items from API:', quotationData.items);
        const items: QuotationItem[] = quotationData.items.map((item: any) => ({
          id: `${item.id}`,
  ...
  if (!quotationData) {
    return (
      <div className="p-6 max-w-[1600px] mx-auto">
        <div className="text-center py-12">
          <p className="text-destructive">报价单不存在</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">修改报价单信息和产品</p>
          </div>
        </div>
        <Button onClick={handleSubmit} disabled={updateQuotationMutation.isPending}>
          {updateQuotationMutation.isPending ? "保存中..." : "保存报价单"}
        </Button>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                已添加 {quotationItems.length} 个产品，总额: {currency} {totalAmount}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    <TableRow className="bg-muted/50 font-medium">
                      ...
                      <TableCell className="text-right font-bold">{currency} {quotationItems.reduce((sum, item) => sum + parseFloat(item.mode === 'batch_selection' ? item.batchSubtotal || '0' : item.fobSubtotal || '0'), 0).toFixed(2)}</TableCell>
                      <TableCell className="text-right font-bold">
                        {quotationItems.reduce((sum, item) => sum + (item.cbm || 0), 0).toFixed(3)} m³
                      </TableCell>
                      <TableCell className="text-right font-bold">
                        {quotationItems.reduce((sum, item) => sum + (item.grossWeight || 0), 0).toFixed(2)} kg
                      </TableCell>
                      <TableCell className="text-right font-bold">
                        {quotationItems.reduce((sum, item) => sum + (item.netWeight || 0), 0).toFixed(2)} kg
                      </TableCell>
                      <TableCell className="text-right font-bold">
