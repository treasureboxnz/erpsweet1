...
export default function OrderCreate() {
  ...
  const createOrderMutation = trpc.orders.create.useMutation({
    onSuccess: (data) => {
      toast.success(`订单创建成功，订单号：${orderNumber}`, {
        duration: 5000, // 显示5秒
      });
      // 跳转到该订单的编辑页面
  ...
  const handleAddFromSearch = async (searchResult: any) => {
    try {
      ...
      const newItem: OrderItem = {
        ...
        productSku: searchResult.productSku || "",
        productImageUrl: searchResult.productImageUrl,
        ...
        batchSubtotal: price,
        materialColor: variant.materialColor?.color || undefined,
        grossWeight: Number(searchResult.grossWeight) || 0,
        netWeight: Number(searchResult.netWeight) || 0,
        cbm: Number(searchResult.cbm) || 0,
      };

      // 检查是否已有相同批次的行，有则叠加数量
      const existingIdx = orderItems.findIndex(
        item => item.mode === 'batch_selection' && item.variantId === searchResult.id
      );
      if (existingIdx >= 0) {
        const updated = [...orderItems];
        const existing = updated[existingIdx];
        const newQty = (existing.batchQuantity || 0) + 1;
        ...
        toast.success(`已将批次 ${searchResult.variantCode} 数量叠加，当前数量: ${newQty}`);
      } else {
        setOrderItems([...orderItems, newItem]);
        toast.success(`已添加批次: ${searchResult.variantCode}`);
      }
    } catch (error) {
  ...
  const handleVariantChange = (variantId: string) => {
    ...
    if (variantData) {
      // 批次模式直接读取批次自身的价格字段（sellingPriceFOB / sellingPriceRMB）
      const price = currency === "USD" 
        ? variantData.variant.sellingPriceFOB?.toString() || "0"
        : variantData.variant.sellingPriceRMB?.toString() || "0";
      console.log('[handleVariantChange] Batch price:', price, 'currency:', currency);
      setBatchUnitPrice(price);
    } else {
  ...
  const handleAddToOrder = () => {
    ...
    if (selectedMode === 'batch_selection') {
      ...
      // 获取批次的包装信息
      const variantWithPackaging = variantsData?.variants.find((v: any) => v.variant.id === selectedVariantId);
      const packagingType = variantWithPackaging?.packagingType;
      const piecesPerBox = variantWithPackaging?.piecesPerBox;

      // 验证一箱多件的订单数量
      if (packagingType === 'bulk' && piecesPerBox && piecesPerBox > 1) {
        if (batchQuantity % piecesPerBox !== 0) {
          toast.error(`订单数量必须是${piecesPerBox}件的倍数（一箱多件包装）`);
          return;
        }
      }
      ...
      console.log('[handleAddToOrder] variantWithMaterial.materialColor:', variantWithMaterial?.materialColor);
      console.log('[handleAddToOrder] variantWithMaterial.materialColor?.color:', variantWithMaterial?.materialColor?.color);
      const materialColor = variantWithMaterial?.materialColor?.color ? {
        colorCode: variantWithMaterial.materialColor.color.colorCode,
        colorName: variantWithMaterial.materialColor.color.colorName ?? null,
        imageUrl: variantWithMaterial.materialColor.color.imageUrl ?? null,
        thumbnailUrl: variantWithMaterial.materialColor.color.thumbnailUrl ?? null,
      } : undefined;
      ...
      const newItem: OrderItem = {
        ...
        materialColor,
        // 添加CBM/毛重/净重/箱数数据
        grossWeight: Number(variantWithPackaging?.totalGrossWeight) || 0,
        netWeight: Number(variantWithPackaging?.totalNetWeight) || 0,
        cbm: Number(variantWithPackaging?.totalCBM) || 0,
        piecesPerBox: variantWithPackaging?.piecesPerBox || 1,
      };
      ...
      const materialColor = defaultVariant?.materialColor?.color ? {
        ...
        colorName: defaultVariant.materialColor.color.colorName ?? null,
        imageUrl: defaultVariant.materialColor.color.imageUrl ?? null,
        thumbnailUrl: defaultVariant.materialColor.color.thumbnailUrl ?? null,
      } : undefined;
    ...
    <div className="p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            已添加 {orderItems.length} 个产品，总额: {currency} {totalAmount.toFixed(2)}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {(() => { console.log('[OrderCreate] Rendering orderItems:', orderItems); return null; })()}
              {orderItems.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  ...
                  <TableBody>
                    {orderItems.map((item) => {
                      console.log('[OrderCreate] Rendering item:', item.id, 'productImageUrl:', item.productImageUrl, 'materialColor:', item.materialColor);
                      return (
                    ...
                    <TableRow className="bg-muted/50 font-medium">
                      ...
                      <TableCell className="text-right font-bold">
                        {orderItems.reduce((sum, item) => sum + (item.cbm || 0) * (item.batchQuantity || item.fobQuantity || 0), 0).toFixed(3)} m³
                      </TableCell>
                      <TableCell className="text-right font-bold">
                        {orderItems.reduce((sum, item) => sum + (item.grossWeight || 0) * (item.batchQuantity || item.fobQuantity || 0), 0).toFixed(2)} kg
                      </TableCell>
                      <TableCell className="text-right font-bold">
                        {orderItems.reduce((sum, item) => sum + (item.netWeight || 0) * (item.batchQuantity || item.fobQuantity || 0), 0).toFixed(2)} kg
                      </TableCell>
                      <TableCell className="text-right font-bold">
